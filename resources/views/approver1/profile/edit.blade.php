...
<div class="max-w-3xl mx-auto mt-10 bg-white border border-blue-800 rounded-lg p-6">
    ...
    <form action="{{ route('approver1.profile.update') }}" method="POST">
        @csrf
        ...
        <div class="mb-4">
            ...
            <input type="text" name="designation" value="{{ old('designation', $approver->designation) }}"
                   class="w-full px-4 py-2 border border-gray-300 rounded">
        </div>

        <div class="mb-4">
            <label class="block text-sm font-medium">Description</label>
            <textarea name="description" class="w-full px-4 py-2 border border-gray-300 rounded">{{ old('description', $approver->description) }}</textarea>
        </div>
